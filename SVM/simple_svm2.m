...
%         K(i,j) = x1(i)*x1(j) + x2(i)*x2(j); % this a linear kernel, it could be more sophisticated
        K(i,j) = exp(-((x1(i)-x1(j))^2 + (x2(i)-x2(j))^2)/lambda); % this a linear kernel, it could be more sophisticated
    end
end
C=1; % a parameter, you actually don't need it for the separable case
alpha = quadprog(diag(y)*K*diag(y),-ones(n,1),eye(n),C*ones(n,1),y',0,zeros(n,1),C*ones(n,1));
...
[A,B] = meshgrid(-1:0.1:3,-0.7:0.1:2.8);
Z = zeros(size(A));
for i=1:size(A,1),
    for j=1:size(A,2),
        v=0;
        for k=1:n,
            if alpha(k)>=eps,
                v = v + y(k)*alpha(k)*exp(-((x1(k)-A(i,j))^2 + (x2(k)-B(i,j))^2)/lambda);
            end
        end
        v = v + b;
        Z(i,j) = v;
